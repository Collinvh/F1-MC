...
public class RaceManager implements CommandUtil {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            if (sender.hasPermission("projectr.fia.race") || sender.hasPermission("projectr.admin")) {
                if (args.length > 0) {
                    switch (args[0]) {
                        case "create":
                            if (args.length > 2) {
                                if (races.get(args[1].toLowerCase()) != null) {
                                    sender.sendMessage(serverPrefix + "Een race met die naam bestaat al.");
                                } else {
                                    try {
                                        ...
                                        if (laps > 0) {
                                            ...
                                            sender.sendMessage(serverPrefix + "Je moet wel meer dan 0 laps invullen");
                                            return true;
                                        }

                                    } catch (NumberFormatException e) {
                                        sender.sendMessage(serverPrefix + "Error reading lapcount.");
                                        return false;
                                    }

                                    sender.sendMessage(serverPrefix + "Race created!");
                        ...
                        case "delete":
                            if (args.length > 1) {
                                if (races.get(args[1].toLowerCase()) != null) {
                                    ...
                                    sender.sendMessage(serverPrefix + "Race gedelete");
                                } else {
                                    sender.sendMessage(serverPrefix + "Race bestaat niet");
                                }
                        ...
                        case "reset":
                            if (runningRace != null) {
                                ...
                                sender.sendMessage(serverPrefix + "Race gereset");
                                return true;
                            } else {
                                sender.sendMessage(serverPrefix + "Er is geen race bezig");
                                return false;
                            }
                        case "result":
                            if (runningRace != null) {
                                sender.sendMessage(serverPrefix + "Race result:");
                                runningRace.getFinishedDrivers().forEach(data -> sender.sendMessage(data.getFinishPosition() + "." + " : " + data.getDriver().getPlayer().getName()));
                            } else {
                                sender.sendMessage(serverPrefix + "Niemand is gefinished!");
                            }
                        case "detectie":
                            if (args.length > 2) {
                                    ...
                                    case "add":
                                        ...
                                        if (obj != null && region != null) {
                                            if (args.length > 3) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Geadd!");
                                            } else {
                                                sender.sendMessage(serverPrefix + "Usage /race detectie add [race] [naam]");
                                            }
                                        } else {
                                            sender.sendMessage(serverPrefix + "Selectie OF race is null");
                                        }
                                        return true;
                                    case "delete":
                                        ...
                                        try {
                                            ...
                                            if (obj != null) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Detectie removed.");
                                            } else {
                                                sender.sendMessage(serverPrefix + "Race is null");
                                            }
                                        } catch (Exception e) {
                                            sender.sendMessage(serverPrefix + args[1] + " is geen nummer.");
                                        }
                                        return true;
                                    case "list":
                                        obj = races.get(args[2]);
                                        if (obj != null) {
                                            if (obj.getStorage().getDetecties().size() > 0) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Er zijn geen detecties.");
                                            }
                                        } else {
                                            sender.sendMessage(serverPrefix + "Race is null");
                                        }
                                ...
                                switch (args[1].toLowerCase()) {
                                    ...
                                    case "set":
                                        if(args.length > 4) {
                                            ...
                                            if(zone == null) {
                                                sender.sendMessage(serverPrefix + "Die zone bestaat nog niet?");
                                                return false;
                                            }
                                            ...
                                            if(region == null) {
                                                sender.sendMessage(serverPrefix + "Selectie is null");
                                                return false;
                                            }
                                            ...
                                            switch (args[4].toLowerCase()) {
                                                case "detectie":
                                                    ...
                                                    sender.sendMessage(serverPrefix + "Zone aangepast.");
                                                    return true;
                                                case "straight":
                        ...
                        case "list":
                            if (races.size() > 0) {
                                sender.sendMessage(serverPrefix + "Racelijst : ");
                                for (RaceObject raceObject : races.values()) {
                                    sender.sendMessage(raceObject.getRaceName() + " | = | " + raceObject.getLapCount());
                                }

                            } else {
                                sender.sendMessage(serverPrefix + "Er zijn geen races aangemaakt");
                            }
                            return true;
                        case "createresult":
                            if(runningRace != null) {
                                if(runningRace.getRunningMode().isHasLaps()) {
                                    sender.sendMessage(serverPrefix + "Unsupported at the moment!");
                                } else {
                                    try {
                                        createResultImage();
                                    } catch (IOException e) {
                                ...
                                sender.sendMessage(serverPrefix + "Geen race bezig.");
                            }
                            break;
                        case "set":
                            if (args.length > 2) {
                                    ...
                                    switch (args[2].toLowerCase(Locale.ROOT)) {
                                        ...
                                        case "sector3":
                                            if (region != null) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Finish is aangepast!");
                                            } else {
                                                sender.sendMessage(serverPrefix + "Je moet wel iets geselecteerd hebben");
                                            }
                                            return true;
                                        case "pitlane":
                                        case "pit":
                                            if (region != null) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Pitlane is aangepast!");
                                            } else {
                                                sender.sendMessage(serverPrefix + "Je moet wel iets geselecteerd hebben");
                                            }
                                            return true;
                                        ...
                                        case "sector1":
                                            if (region != null) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Sector 1 is aangepast!");
                                            } else {
                                                sender.sendMessage(serverPrefix + "Je moet wel iets geselecteerd hebben");
                                            }
                                            return true;
                                        ...
                                        case "sector2":
                                            if (region != null) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Sector 2 is aangepast!");
                                            } else {
                                                sender.sendMessage(serverPrefix + "Je moet wel iets geselecteerd hebben");
                                            }
                                            return true;
                                        case "pitexit":
                                            if (region != null) {
                                                ...
                                                sender.sendMessage(serverPrefix + "Pit Exit is aangepast!");
                                            } else {
                                                sender.sendMessage(serverPrefix + "Je moet wel iets geselecteerd hebben");
                                            }
                                            return true;
                                        case "laps":
                                            try {
                                                ...
                                                sender.sendMessage(serverPrefix + "Laps zijn aangepast!");
                                            } catch (NumberFormatException e) {
                                                sender.sendMessage(serverPrefix + args[3] + " is geen nummer!");
                                            }
                                            return true;
                                    }
                                }
                            ...
                            sender.sendMessage(serverPrefix + "Usage /race set [naam] [segment] {s1/s2/s3/pit/pitexit/laps}");
                            return true;
                        case "start":
                            if (args.length > 2) {
                                ...
                                if (object != null) {
                                    ...
                                    sender.sendMessage(serverPrefix + "Race gestart.");
                                    runningRace = object;
                                } else {
                                    sender.sendMessage(serverPrefix + "Race bestaat niet!");
                                }
                            } else {
                                sender.sendMessage(serverPrefix + "/race start [naam] [id]");
                            }
                            return true;
                        case "stop":
                            if (runningRace != null) {
                                ...
                                runningRace = null;
                                sender.sendMessage(serverPrefix + "Race gestopt.");
                            } else {
                                sender.sendMessage(serverPrefix + "Er word geen race gerunned.");
                            }
                            return true;
                    }
                } else {
                ...
                sender.sendMessage(serverPrefix + "Je hebt hier geen permissie voor.");
                return true;
            }
        } else {
            sender.sendMessage(serverPrefix + "Op dit moment kunnen alleen spelers dit.");
            return false;
        }
        return true;
    }
    ...
    private void createResultImage() throws IOException {
        ...
        if(drivers.values().toArray().length > 0) {
                ...
                treeMap.forEach((driver, aLong) -> {
                    if (driver.getLapstorage().getBestTime() != null) {
                        ...
                        if (pos.get() < 21) {
                            ...
                            try {
                                File file = new File(ProjectR.getRacing().getDataFolder() + "/storage/tyre/" + driver.getLapstorage().getBestTime().getTyre().getTyreID() + ".png");
            ...
            ip.dispose();
            File file = new File(ProjectR.getRacing().getDataFolder() + "/temp/result.png");
            file.mkdir();
            ImageIO.write(image, "png", file);

            file = new File(ProjectR.getRacing().getDataFolder() + "/temp/result.png");
    ...
    public static void saveRaces() {
        for(RaceObject raceObj : races.values()) {
            File raceLoc = Paths.get(ProjectR.getRacing().getDataFolder() + "/storage/races/" + raceObj.getRaceName() + ".json").toFile();
    ...
    public static void loadRaces() {
        File teamLoc = Paths.get(ProjectR.getRacing().getDataFolder() + "/storage/races/").toFile();
        if(teamLoc.exists()) {
