package collinvht.projectr.commands.racing.setup;
...
public class SetupManager {
    ...
    public static void saveSetups() {
        setups.forEach((uuid, obj) -> {
            File setupLoc = Paths.get(ProjectR.getRacing().getDataFolder().toString() + "/storage/player/" + uuid.toString() + ".json").toFile();
    ...
    public static void loadSetups() {
        File playerStorage = Paths.get(ProjectR.getRacing().getDataFolder() + "/storage/player/").toFile();
        if(playerStorage.exists()) {
            if(playerStorage.listFiles() != null) {
