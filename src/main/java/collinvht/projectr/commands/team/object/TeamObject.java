package collinvht.projectr.commands.team.object;
...
public class TeamObject {
    ...
    private static final String prefix = ProjectR.getServerPrefix();
    ...
    public void inviteMember(CommandSender sender, UUID uuid) {
        if(invited.contains(uuid)) {
            sender.sendMessage(prefix + "Die speler heb jij al geinvite!");
        } else {
            invited.add(uuid);
            sender.sendMessage(prefix + "Speler geinvite!");

            Player player = Bukkit.getPlayer(uuid);
            if(player != null) {
                player.sendMessage(prefix + getTeamName() + ", heeft je uitgenodigd om hun team te joinen!");
            }
        }
    }
    ...
    public void addMember(CommandSender sender, UUID uuid) {
        ...
        if(sender != null) {
            sender.sendMessage(prefix + "Speler geadd!");
        }
    }
    ...
    public void removeMember(CommandSender sender, UUID uuid) {
        ...
        if(sender != null) {
            sender.sendMessage(prefix + "Speler verwijderd!");
        }
    }
    ...
    public void setGolfcooldown(int cooldown) {
        ...
        }.runTaskTimer(ProjectR.getRacing(), 0, 15);
    }
